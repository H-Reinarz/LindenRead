...
for (year in YEARS){
  ...
  files <- list.files(folderPath, pattern = glob2rx("2013*CNR4*.txt"), 
                      full.names = T)
  ...
  awb <- lapply(files, function(x){
    ...
    solarNoonHours <- as.numeric(strftime(solarNoon, format="%H", tz ="UTC"))
    solarNoonMinutes <- as.numeric(strftime(solarNoon, format="%M", tz ="UTC"))
    solarNoonSeconds <- as.numeric(strftime(solarNoon, format="%S", tz ="UTC"))
    solarNoonDecode <- solarNoonHours * 3600 + solarNoonMinutes * 60 + solarNoonSeconds
    
    # Filtern des Datensatzes auf die Zenithzeit
    if (length(subsetCNR[subsetCNR$TIMEDECODE == solarNoonDecode,]$TIMESTAMP) == 0){
      subsetCNR <- subsetCNR[subsetCNR$TIMEDECODE == min(subsetCNR$TIMEDECODE),]
    } else {
      subsetCNR <- subsetCNR[subsetCNR$TIMEDECODE == solarNoonDecode,]
    }
...
write.table(zenithFrame, "h:/Geography/aerosol/Data/zwischenergebnis/CNR4-2013.dat", row.names = F, quote = F, sep = ";")
# Plot
